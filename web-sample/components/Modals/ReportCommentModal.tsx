...
const ReportCommentModal = ({ modal, onClose }: ReportCommentModalProps) => {
  const comment = (modal.data as any)?.comment;
  ...
  const onSubmit = async (values: ReportFormValues) => {
    ...
    try {
      const response: any = await commentService.reportComment(
        comment.id,
        values.reason,
      );
      ...
      if (
        ...
        ToastService.success(
          "Comment reported successfully. Thank you for your feedback!",
        );
        onClose();
      } else {
        ToastService.error(response?.message || "Failed to report comment");
      }
    } catch (error: any) {
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[360px] px-6 py-8 bg-white border-0 shadow-xl rounded-3xl overflow-hidden items-center z-[100]">
        <DialogTitle className="sr-only">Report Comment</DialogTitle>
