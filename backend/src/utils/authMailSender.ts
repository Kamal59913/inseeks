...
const authMailSender = async (
  ...
  try {
    ...
    const mg = mailgun.client({
      username: "api",
      key: process.env.AUTH_MAILGUN_API_KEY || "",
    });

    const domain = process.env.AUTH_MAILGUN_DOMAIN || "";

    const from = process.env.AUTH_MAILGUN_EMAIL_USER;

    // console.log(domain, from, process.env.AUTH_MAILGUN_API_KEY);
    // const data =
    // console.log(data);
