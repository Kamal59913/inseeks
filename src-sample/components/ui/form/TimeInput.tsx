...
interface TimeInputProps {
 ...
 const TimeInput: FC<TimeInputProps> = ({
   ...
   if (variant === "transparent") {
     inputClasses = cn(
       "w-full text-center bg-transparent text-white text-[20px] px-0 py-3 focus:outline-none placeholder-gray-500 caret-white",
       disabled && "opacity-40 cursor-not-allowed",
       className
     );
   } else {
     inputClasses = cn(
       "w-full text-[16px] md:text-sm rounded-lg p-3 bg-[#FFFFFF0D] border border-white/5 opacity-80",
       disabled && "opacity-40 bg-gray-800 cursor-not-allowed",
       className
     );
   }
