...
export const validationUtils = {
  ...
  password: (fieldName: string) =>
    z
      ...
      .max(20, `${fieldName} password must not exceed 20 characters`)
  ...
  passwordCustomV2: (fieldName: string) =>
    z
      .string()
      .min(1, `Please enter ${fieldName}`)
      .superRefine((val, ctx) => {
        const hasLengthAndMix =
          val.length >= 8 && /[a-zA-Z]/.test(val) && /\d/.test(val);
        const hasUpperCase = /[A-Z]/.test(val);

        if (!hasLengthAndMix || !hasUpperCase) {
          ctx.addIssue({
            code: "custom",
            message: "Please complete the password requirements",
          });
        }
      }),
  ...
  customDuration: (fieldName: string) =>
    z.string().superRefine((val, ctx) => {
      const trimmedVal = val.trim();
