...
import { useGetCustomers } from "@/hooks/queries/customers/useGetCustomers";
...
const CustomersTable: React.FC<CustomersTableProps> = ({
  ...
  const { data: localData, isPending } = useGetCustomers(
    {
      page: currentPage,
      limit,
    },
  ...
  const deleteCustomer = async (uuid: string) => {
    try {
      ...
      if (response.status === 200) {
        ...
        await queryClient.invalidateQueries({ queryKey: ["get-customers"] });
        close();
      } else {
        ToastService.error(
    ...
    },
    {
      ...
      accessor: (user) => (
        <div className="flex gap-2">
          <ActionIcons
            onView={() => navigate(`/customers/${user.uuid}`)}
            onDelete={() => {
              open("delete-action", {
  ...
  return (
    <DataTable
      ...
      limit={limit}
      onRowClick={(user) => navigate(`/customers/${user.uuid}`)} // Add this line
      minWidth="1000px"
    />
  );
};
