...
const buildLengthMessage = (label, type, value) =>
  `Please enter ${type === "select" ? "a valid" : ""} ${label}`.replace(
    /\s+/g,
    " ",
  ).trim();
...
const fileField = (label, { required = true } = {}) =>
...
const filesField = (label, { required = true, min = 1 } = {}) =>
...
const passwordField = (label = "password", { min = 6, max = 64 } = {}) =>
  z
    .string()
    .min(1, `Please enter ${label}`)
    .min(min, `${label.charAt(0).toUpperCase() + label.slice(1)} must be at least ${min} characters`)
    .max(max, `${label.charAt(0).toUpperCase() + label.slice(1)} must be no more than ${max} characters`);

export const validationUtils = {
  stringField,
  optionalStringField: (label, options = {}) =>
...
export const preprocessTrimmedFormData = (values) =>
  Object.fromEntries(
