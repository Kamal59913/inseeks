...
import { postVideoSchema } from '../../utils/formSchemas';
import { preprocessTrimmedFormData } from '../../utils/formValidation';
import { useModalData } from '../../store/hooks';
import AppModal from '../Modal/AppModal';

export function PostVideo({ modalId, data }) {
  const modal = useModalData()
  const postVideoUrl = `${process.env.REACT_APP_API_URL}/createpost/videopost`
  const [loader, setLoader] = useState(false)
  const payload = data || {}
  const { control, handleSubmit, watch, reset } = useAppForm({
    schema: postVideoSchema,
    defaultValues: { description: '', video: null },
  })
  ...
  const onSubmit = (values) => {
    setLoader(true)
    const { description, video: uploadedVideo } = preprocessTrimmedFormData(values)
            ...
            <div className="space-y-3">
              ...
              <div className='mt-4'>
                ...
                <span className="ml-4 inline-flex">
                  <FormFileField control={control} name="video" accept="video/mp4,video/x-m4v,video/*"/>
                </span>
                {loader && <>
              ...
              <div className='h-[180px] w-full bg-slate-600 mt-1 flex justify-center'>
                {previewUrl && <video controls className='bg-slate-200 h-full w-64'><source src={previewUrl} type='video/mp4'></source></video>}
              </div>
