...
export default function ReplaceImages({ modalId, data }) {
  const modal = useModalData()
  const updateAvatarUrl = `${process.env.REACT_APP_API_URL}/users/updateavatar`
  ...
  const onSubmit = (values) => {
    ...
    axios.patch(updateAvatarUrl, formData, {
            ...
            <div className="space-y-3">
              <div className='mt-3'>
                ...
                <span className="ml-4 inline-flex">
                  <FormFileField control={control} name="avatar" accept="image/*"/>
                </span>
                {loader && <>
              ...
              <div className='h-[220px] w-full bg-slate-600 mt-1 flex justify-center'>
                {previewUrl && <img className='bg-slate-200 h-full w-64 rounded-2xl' src={previewUrl} alt=''/>}
              </div>
