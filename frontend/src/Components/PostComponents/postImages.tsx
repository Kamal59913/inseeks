...
export default function PostImages(props: any) {
  ...
  return (
    <div className="bg-[#111827] rounded-2xl overflow-hidden transition-all duration-200 w-full animate-fade-in">
      ...
      {props.title && (
        <p className="px-4 pb-2 text-base font-semibold text-slate-100">
          {props.title}
        </p>
      )}
      ...
      {images.length > 0 && (
        <div
          className={`px-4 pb-3 grid gap-1.5 ${images.length === 1 ? "grid-cols-1" : images.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}
        >
      ...
      <div className="flex items-center justify-between px-4 py-3 ">
        ...
        <div className="flex items-center gap-3 text-xs text-slate-500">
          ...
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              copyToClipboard(`${window.location.origin}/post/${props.postId}`);
            }}
