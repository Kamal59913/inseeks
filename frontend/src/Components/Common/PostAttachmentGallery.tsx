...
const normalizeAttachments = (
  ...
  const isImage = /\.(png|jpe?g|gif|webp|svg|bmp|avif)(\?|$)/.test(
    normalizedUrl,
  );
  ...
  >
    ...
    <div className="min-w-0 flex-1">
      ...
      <p className="text-xs text-slate-400">
        {formatBytes(attachment.bytes) || "Open file"}
      </p>
    </div>
        ...
        {items.map((attachment, index) => {
          ...
          if (kind === "audio") {
            return (
              <div
                key={`${attachment.url}-${index}`}
                className="rounded-2xl bg-[#0f172a] p-3"
              >
    ...
    <div className="px-4 pb-3">
      ...
      {primaryKind === "video" ? (
        <video
          ...
          controls
          onClick={() => window.open(primary.url, "_blank")}
          className="h-52 w-full rounded-xl bg-black object-cover cursor-pointer hover:opacity-90 transition-opacity"
        />
      ) : null}
      ...
      {primaryKind === "file" || primaryKind === "pdf" ? (
        <FileCard attachment={primary} />
      ) : null}
